...
def is_send_weibo_time():
    ...
    if (time_now.tm_hour == 8) and (time_now.tm_min >= 30) and (time_now.tm_min <= 50):
        return True
    return False
# 定义要爬取的微博大V的微博ID
id='1896820725'
...
def send_weibo_to_mail():
    ...
    id2 = "5384479977"
    name2 = get_userInfo(id2)
    file2 = "bubai"+".txt"
    get_weibo(id2,file2)

    os.system("rm -rf weibo.txt")
    os.system("touch weibo.txt")
    os.system("echo 'Weibo:\n\n' > weibo.txt" )
    os.system("echo guxia: >> weibo.txt")
    os.system("cat %s >> weibo.txt"  % (file))
    os.system("echo '\n\n\nbubai:' >> weibo.txt")
    os.system("cat %s >> weibo.txt"  % (file2))
    os.syste?!?jedi=1, m("cat weibo.tx?!? (*_*command*_*) ?!?jedi?!?"t | mutt -s '新闻信息早读' scutqyh@163.com")
    os.system("rm -rf weibo.txt bubai.txt guxia.txt")
    print('finish')

def send_weibo_mail_on_time():
    while True:
        if is_send_weibo_time() == True:
            send_weibo_to_mail()
            time.sleep(60 * 30)
        else:
            time.sleep(60 * 5)
if __name__=="__main__":
    ...
    os.system("echo '\n\n\nbubai:' >> weibo.txt")
    os.system("cat %s >> weibo.txt"  % (file2))
    os.system("cat weibo.txt | mutt -s '新闻信息早读' scutqyh@163.com")
    print('finish')
